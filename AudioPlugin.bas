...
Private Const MIXERLINE_DEVICETYPES = "FIRST OR UNDEFINED,DIGITAL,LINE,MONITOR,SPEAKERS,HEADPHONES,TELEPHONE,WAVEIN,VOICEIN OR LAST"
...
Private uMixerControls(0 To 20, 0 To 20)     As MIXERCONTROL ' local array to store mixer controls
...
Public Function GetDeviceNameType(DeviceNumber As Long) As String
    GetDeviceNameType = Split(MIXERLINE_DEVICETYPES, ",")(DeviceNumber)
End Function


Public Function OpenMixer(ByVal DeviceType As Long, ByVal DeviceNumber As Long) As Long

    Dim ret             As Long
    
    ' is there a mixer available?
    'MsgBox mixerGetNumDevs
    If DeviceType < 0 Or DeviceType > mixerGetNumDevs - 1 Then Exit Function
    
    ' open the mixer
    'ret = mixerOpen(hMixerHandle, MixerNumber, 0, 0, 0)
    ret = mixerOpen(hMixerHandle, DeviceType, 0, DeviceNumber, 0)
    If ret <> MMSYSERR_NOERROR Then Exit Function

    ' get the primary line controls by name, (this does not get all of the controls).
    
    ret = GetMixerControl(hMixerHandle, DeviceNumber, MIXERCONTROL_CONTROLTYPE_VOLUME, uMixerControls(SPEAKER, DeviceNumber))

    ret = GetMixerControl(hMixerHandle, DeviceNumber, MIXERCONTROL_CONTROLTYPE_MUTE, uMixerControls(SPEAKER, DeviceNumber + 10))
    
    
'    Select Case MixerNumber
'        Case SPEAKER
...
'
'        Case LINEIN
'            ret = GetMixerControl(hMixerHandle, MIXERLINE_COMPONENTTYPE_DST_HEADPHONES, MIXERCONTROL_CONTROLTYPE_VOLUME, uMixerControls(LINEIN))
'
'        Case MICROPHONE
...
Public Function SetVolume(Control As VOL_CONTROL, ByVal NewVolume As Long, DeviceNumber As Long) As Long

    SetVolume = SetControlValue(hMixerHandle, uMixerControls(Control, DeviceNumber), NewVolume)

End Function

Public Function GetVolume(Control As VOL_CONTROL, DeviceNumber As Long) As Long

    GetVolume = GetControlValue(hMixerHandle, uMixerControls(Control, DeviceNumber))

End Function

Public Function SetMute(Control As MUTE_CONTROL, ByVal MuteState As Boolean, DeviceNumber As Long) As Boolean
    
    Dim mute    As Long
    mute = Abs(MuteState)
    
    SetMute = SetControlValue(hMixerHandle, uMixerControls(Control, DeviceNumber + 10), mute)
    
End Function

Public Function GetMute(Control As MUTE_CONTROL, DeviceNumber As Long) As Boolean

    GetMute = CBool(-GetControlValue(hMixerHandle, uMixerControls(Control, DeviceNumber + 10)))
...
On Error GoTo Exception
    ...
    Let rc = mixerGetLineInfo(9, pmxl, MIXER_GETLINEINFOF_COMPONENTTYPE)
...
Private Function GetMixerControl(ByVal hMixer As Long, ByVal componentType As Long, ByVal ctrlType As Long, ByRef mxc As MIXERCONTROL) As Long
    ...
    'Debug.Print s(mxl.szName)
    'Debug.Print s(mxl.szShortName)
